...
public class EndToEndMultipleClientsGetPutDel {
	...
	private void setUpSlave(String name) throws UnknownHostException, IOException, KVException {
		...
		KVServer slaveKvs = new KVServer(1, 1);
	...
	public void testPutGet() throws InterruptedException, KVException, UnknownHostException {
		...
		KVClient client2 = new KVClient(InetAddress.getLocalHost().getHostAddress(), 8888);
		...
		try {
			...
			client2.put("key4", "value4");
		} catch (Exception e) {
			e.printStackTrace();
			System.out.println("INFO EndToEnd.testPutGet: Finished.");
			fail("put failed");
		}
		//test 4 gets work correctly
		try {
			assertEquals("TEST 1 get failed", client2.get("key1"), "value1");
			assertEquals("TEST 2 get failed", client.get("key2"), "value2");
			assertEquals("TEST 3 get failed", client2.get("key3"), "value3");
			assertEquals("TEST 4 get failed", client.get("key4"), "value4");
		} catch (Exception e) {
			System.out.println("INFO EndToEnd.testPutGet: Finished.");
			e.printStackTrace();
		}
		...
		try {
			...
			client2.put("key2", "value2new");
			client.put("key3", "value3new");
			client2.put("key4", "value4new");
		}
		catch (Exception e) {
			e.printStackTrace();
		}
		...
		try {
			assertEquals("TEST 1new get failed", client2.get("key1"), "value1new");
			assertEquals("TEST 2new get failed", client.get("key2"), "value2new");
			assertEquals("TEST 3new get failed", client2.get("key3"), "value3new");
		...
		//make sure deleted values are actually gone
		try {
			assertEquals("TEST 1new get failed", client.get("key1"), "");
			assertEquals("TEST 2new get failed", client2.get("key2"), "");
			assertEquals("TEST 3new get failed", client.get("key3"), "");
			assertEquals("TEST 4new get failed", client2.get("key4"), "");
		} catch (Exception e) {
			System.out.println("INFO EndToEnd.testPutGet: Finished.");
			e.printStackTrace();
		}
		
		//overwriting values of seperate clients
		try {
			client.put("key1", "a");
			client2.put("key1", "1");
			client.put("key2", "b");
		...
		try {
			assertEquals("TEST 1new get failed", client2.get("key1"), "1");
			assertEquals("TEST 2new get failed", client2.get("key2"), "2");
			assertEquals("TEST 3new get failed", client2.get("key3"), "3");
			assertEquals("TEST 4new get failed", client2.get("key4"), "4");
		} catch (Exception e) {
			System.out.println("INFO EndToEnd.testPutGet: Finished.");
			e.printStackTrace();
		}
