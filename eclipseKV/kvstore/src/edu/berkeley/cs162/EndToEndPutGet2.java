...
public class EndToEndPutGet2 {
	...
	public void testPutGet() throws InterruptedException, KVException, UnknownHostException {
		...
		//Proper delete acknowledged
		try {
			assertEquals("TEST 2 get failed", client.get("foo"), "");
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
		...
		try {
			client.put("moo",  "zoo");;
			client.put("moo", "boo");
			assertEquals("TEST 3 get failed", client.get("moo"), "boo");
		} catch (Exception e) {
			System.out.println("INFO EndToEnd.testPutGet: Finished.");
			e.printStackTrace();
		}
		...
		//get nothing 
		try {
			System.out.println("Getting Empty Key");
			assertEquals("TEST 4 get failed", client.get("random"), "");
		}
		catch (Exception e) {
			e.printStackTrace();
		} finally {
