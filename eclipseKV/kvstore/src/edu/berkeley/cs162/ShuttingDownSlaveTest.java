...
public class ShuttingDownSlaveTest {
	...
	public void setUpSlave(String name) throws UnknownHostException, IOException, KVException {
	...
	public void testPutGet() throws InterruptedException, KVException, UnknownHostException {
		...
		try {
			...
			System.out.println(slaveRunners.size());
			
			int count = 1;
			String sleepingSlave = null;
			ServerRunner aliveSlave = null;
			for(Entry<String, ServerRunner> slaveServer :  slaveRunners.entrySet()) {
				if (count > 0){
					ServerRunner currentServer = slaveServer.getValue();
					currentServer.stop();
					sleepingSlave = currentServer.runnerName;
					count--;
				}
				else {
					aliveSlave  = slaveServer.getValue();
				}
			}
